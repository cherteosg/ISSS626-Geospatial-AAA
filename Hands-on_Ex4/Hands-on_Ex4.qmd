...
1) import geospatial data using appropriate function(s) of sf package,
2) import csv file using appropriate function of readr package,
3) perform relational join using appropriate join function of dplyr package,
4) compute spatial weights using appropriate functions of spdep package, and
5) calculate spatially lagged variables using appropriate functions of spdep package.
...
Ensure that spdep, sf, tmap and tidyverse packages of R are installed.
...
Import hunan shapefile into R using st_read() of sf package. The imported shapefile will be simple features Object of sf.
...
Import hunan_2012.csv into R by using read_csv() of readr package. The output is R dataframe class.
...
E.g. To see the neighbors for the first polygon in the object, type:

```{r}
wm_q[[1]]
```
The above shows that Polygon 1 has 5 neighbors. The numbers represent the polygon IDs as stored in hunan SpatialPolygonsDataFrame class.

Retrieve the county name of Polygon ID=1 by using the code chunk below:
...
Reveal the county names of the five neighboring polygons by using the code chunk below:
...
Warning: The output might run over several pages so take note when printing the report. 

### Creating (ROOK) Contiguity Based Neighbours

Compute Rook contiguity weight matrix using the code chunk below:
...
We will need points to associate with each polygon before we can make our connectivity graph. It is a little more complicated than just running st_centroid on the sf object: us.bound. We need the coordinates in a separate data frame for this to work. 
...
You can improve visibility by changing the colors and enlarging the points('cex'). 

```{r}
plot(hunan$geometry, border="steelblue")
...
Answer: The average number of neighbouring counties per county.
...
Another option to display the structure of the weight matrix is to combine table() and card() of spdep.

```{r}
table(hunan$County, card(wm_d62))
```

Check if the spatial weights matrix (wm_d62) forms one single connected network, or is split into multiple disconnected groups (components) of regions using the code chunk below.
...
Show how many regions connect to each connected network. 
...
Note: Each county has 6 neighbours.
...
Next, we need to assign weights to each neighbouring polygon. In our case, each neighbouring polygon will be assigned equal weight (style=“W”). This is accomplished by assigning the fraction 1/(#ofneighbors) to each neighbouring county then summing the weighted income values. 

While this is the most intuitive way to summaries the neighbours’ values it has one drawback in that polygons along the edges of the study area will base their lagged values on fewer polygons thus potentially over- or under-estimating the true nature of the spatial autocorrelation in the data. 
...
See the weight of the first polygon’s eight neighbours type:
...
Question: What does Spatial lag with row-standardized weights mean now?

Answer: Averaging neighbours' values with equal weight using row standardization.
...
Calculate spatial lag as a sum of neighboring values by assigning binary weights using glist = in the nb2listw function.

Apply a function that will assign a value of 1 per each neighbour using lapply. This applies a function across each value in the neighbors structure.
...
Question: Do you understand the meaning of Spatial lag as a sum of neighboring values now?

Answer: More neighbours increases the spatial lag value simply because it is based on summation even if individual values are not higher. 

Next, append the lag_sum GDPPC field into hunan sf data frame by using the code chunk below.
