...
ii.map <- tm_shape(lisa) + 
  tm_polygons(fill = "ii", 
              ...
              fill.legend = tm_legend( 
                ...
                position = tm_pos_out())) + 
  tm_borders(fill_alpha = 0.5) + 
  tm_title("Local Morans'I of GDPPC (Queen's method)")
...
**Answer**
The LISA maps show that spatial autocorrelation in GDP per capita is localised. Statistically significant (p < 0.05) High-High clusters are found in the northeastern part of Hunan, indicating counties with high GDP per capita surrounded by similarly high-value neighbours. A significant Low-Low cluster appears in the southwestern part of the province, indicating low-GDP counties surrounded by other low-GDP counties. A few Low-High outliers occur next to the northeastern cluster, while no High-Low outliers are found. Most counties are not statistically significant, so the global clustering is driven mainly by these few local clusters.
